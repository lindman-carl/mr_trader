...
def main():
    ...
    print(f"Starting Airflow Scheduler with AIRFLOW_HOME={airflow_home}")
    
    try:
        subprocess.run(["airflow", "scheduler"], env=env, check=True)
        
    except KeyboardInterrupt:
        print("\nStopping Airflow Scheduler...")
        
    except subprocess.CalledProcessError as e:
        print(f"Error running Airflow Scheduler: {e}")
        sys.exit(1)
